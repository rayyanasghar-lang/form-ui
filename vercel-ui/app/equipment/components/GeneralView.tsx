...
import type { GeneralRecord } from "../types/records"
...
interface GeneralViewProps {
  ...
  label: string
  apiPath: string
  onEdit: (item: GeneralRecord) => void
  onView: (item: GeneralRecord) => void
  refreshKey: number
}

export default function GeneralView({ type, label, apiPath, onEdit, onView, refreshKey }: GeneralViewProps) {
  ...
  const { data: queryData, isLoading: loading } = useEquipment({
    type: apiPath, // GeneralView uses apiPath as type/endpoint
    page,
    limit,
              ...
              <TableBody>
                {data.map((item: GeneralRecord) => (
                  ...
                  >
                    ...
                    <TableCell className="font-medium">{item.name || item.model || "-"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
