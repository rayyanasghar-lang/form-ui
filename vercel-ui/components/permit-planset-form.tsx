...
export default function PermitPlansetForm() {
      ...
      if (result.success && result.data) {
        setFormData((prev) => ({
          ...
          companyLogo:
            ...
            result.data.logo ||
            result.data.image_1920 ||
            result.data.image_128 ||
            prev.companyLogo,
          contactName: result.data.name || prev.contactName,
          email: result.data.email || prev.email,
          phone: result.data.phone || prev.phone,
          companyAddress:
            result.data.contact_address ||
            result.data.address ||
            prev.companyAddress,
        ...
        if (stored) {
          try {
            ...
            setFormData((prev) => ({
              ...
              companyAddress:
                c.contact_address || c.address || prev.companyAddress,
      ...
      const timeout = setTimeout(async () => {
        ...
        if (res.success && res.lat && res.lng) {
          setFormData((prev) => {
            ...
            if (
              Math.abs(currentLat - res.lat!) < 0.0001 &&
              Math.abs(currentLng - res.lng!) < 0.0001
            ) {
              return prev;
            }
            return {
      ...
      if (res.success && projectId) {
        ...
        const errorMsg = res.error
          ? typeof res.error === "object"
            ? (res.error as any).message || (res.error as any).status
            : String(res.error)
          : "Draft saved but ID missing";
  ...
  const prepareProjectPayload = (
    ...
    const contractorStr =
      typeof window !== "undefined" ? localStorage.getItem("contractor") : null;
