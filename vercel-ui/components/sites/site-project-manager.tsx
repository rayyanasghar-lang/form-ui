...
import {
  useSaveAnswers,
  useSite,
  useServiceQuestionsUnion,
  useProject,
} from "@/hooks/use-site-queries";
...
export function SiteProjectManager({
  initialSiteUuid,
}: SiteProjectManagerProps = {}) {
  const [step, setStep] = useState<WorkflowStep>(
    initialSiteUuid ? "dashboard" : "initial",
  );
  const [siteUuid, setSiteUuid] = useState<string | null>(
    initialSiteUuid || null,
  );
    ...
    if (ids.length > 0) {
      ...
      try {
        ...
        toast.error("Cloud sync failed", {
          description: e.message || "Connection error",
        });
      } finally {
    ...
    try {
      ...
      if (result.status === "success") {
        ...
        toast.error("Failed to save", {
          description: result.message || "Validation error",
        });
      }
    } catch (error: any) {
        ...
        {step === "initial" && (
          ...
          >
            ...
            <h1 className="text-6xl font-black text-zinc-900 mb-6 tracking-tighter">
              Sun<span className="text-primary italic">Permit</span>
            </h1>
            <p className="text-xl text-zinc-500 max-w-lg text-center mb-10 leading-relaxed">
              Start by creating a new site or selecting an existing location to
              begin the engineering workflow.
            </p>
            <Button
              size="lg"
          ...
          >
            <div className="mb-12 flex items-center justify-between">
              <div>
                ...
                <p className="text-zinc-500 font-medium">
                  Please provide the technical details required for{" "}
              ...
              <Button
                ...
                className="text-zinc-400 hover:text-primary font-bold"
              >
            ...
            <div className="w-full">
              {questionsLoading ? (
                ...
                <div className="w-full">
                  {selectedServiceIds.includes("1") ||
                  selectedServiceIds.includes("permit-planset") ? (
