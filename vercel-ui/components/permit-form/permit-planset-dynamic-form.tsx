...
import { DynamicFormEngine, DynamicFormEngineHandle } from "@/components/forms/dynamic-form-engine"
import { Question, SiteFullData } from "@/types/site-centric"
import { Card } from "@/components/ui/card"
import { ClipboardList, HardHat, Zap, Compass, Loader2 } from "lucide-react"
...
export default function PermitPlansetDynamicForm({ siteUuid, projectUuid }: PermitPlansetDynamicFormProps) {
      ...
      try {
        ...
        if (res.status === "success" && res.data) {
          ...
          const defaults: Record<string, any> = {
            ...
            has_battery_backup: site.answers?.has_battery_backup === true ? "yes" : (site.answers?.has_battery_backup === false ? "no" : undefined),
          ...
          if (site.address && site.address.length > 5) {
            console.log("[PermitPlanset] Triggering automated lookups for:", site.address)
  ...
  const allQuestions: Question[] = useMemo(() => [
    ...
    },
    {
        ...
        sequence: 45,
        options: [
            { id: 2051, value: "shingle", label: "Asphalt Shingle", image: "/asphalt.webp" },
            { id: 2052, value: "metal", label: "Metal Roof", image: "/standing.jpg" },
    ...
    },
    {
        ...
        sequence: 45.1,
        options: [
            { id: 20551, value: "standing_seam", label: "Standing Seam", image: "/standing.jpg" },
            { id: 20552, value: "metal_shingles", label: "Metal Shingles", image: "/shingle.jpg" },
            { id: 20553, value: "corrugated_metal", label: "Corrugated Metal", image: "/corruted.jfif" }
        ]
    },
    {
        id: 206,
  ...
  const questions = useMemo(() => {
    ...
    const hiddenKeys = ["project_name", "address", "project_type", "system_size"]
    ...
    <div className="w-full" data-no-loader="true">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 pb-10">
          <InfoCard 
            ...
            title="Structural" 
            desc="Ground-mount fields appear automatically based on system type."
            color="orange"
          />
          <InfoCard 
            icon={<Zap className="w-5 h-5" />} 
            title="Electrical" 
            desc="Service voltage and panel ratings mapped directly to electrical models."
            color="blue"
          />
          <InfoCard 
            icon={<Compass className="w-5 h-5" />} 
            title="Jurisdiction" 
            desc="AHJ and Utility records are cross-referenced for plan accuracy."
            color="green"
          />
      </div>
    </div>
...
function InfoCard({ icon, title, desc, color }: { icon: any, title: string, desc: string, color: string }) {
    ...
    return (
        <Card className="p-6 rounded-[32px] border-none shadow-xl bg-white/60 backdrop-blur-lg">
            ...
            <p className="text-sm font-bold text-zinc-400 leading-relaxed">{desc}</p>
        </Card>
    )
}
